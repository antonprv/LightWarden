...
namespace Code.Gameplay.Common.Physics
{
  public class PhysicsService : IPhysicsService
  {
    ...
    {
      ContactFilter2D filter = CreateFilter(layerMask);

      int hitCount = Physics2D.Raycast(worldPosition, direction, filter, Hits);
    ...
    public GameEntity LineCast(Vector2 start, Vector2 end, int layerMask)
    {
      ...
      int hitCount = Physics2D.Raycast(start, direction, filter, Hits, direction.magnitude);
    ...
    {
      ContactFilter2D filter = CreateFilter(layerMask);
      int hitCount = Physics2D.OverlapPoint(worldPosition, filter, OverlapHits);
    ...
    public int OverlapCircle(Vector3 worldPos, float radius, Collider2D[] hits, int layerMask) =>
        Physics2D.OverlapCircle(worldPos, radius, CreateFilter(layerMask), hits);
    ...
    private static ContactFilter2D CreateFilter(int layerMask)
    {
